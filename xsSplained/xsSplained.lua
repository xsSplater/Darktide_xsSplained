...
local bg_x_pad = 50
local bg_y_pad = 10 -- 10
local entry_size = { 750, 35 } -- 750, 40
local vert_spacing = 25 + entry_size[2]
...
mod:hook_safe(CLASS.InventoryBackgroundView, "_setup_top_panel", function(self, ...)
	local mechanics_tuples = {
		...
		},
		{
			...
			mechanics_title = mod:localize("finesse_title"),
			mechanics_txt = mod:localize("finesse_description"),
		},
		{
		...
		},
		{
			...
			mechanics_title = mod:localize("rending_title"),
			mechanics_txt = mod:localize("rending_description"),
		},
		-- {
			-- mechanics_key = "cleave",
			-- mechanics_title = mod:localize("cleave_title"),
			-- mechanics_txt = mod:localize("cleave_description"),
		-- },
	}
